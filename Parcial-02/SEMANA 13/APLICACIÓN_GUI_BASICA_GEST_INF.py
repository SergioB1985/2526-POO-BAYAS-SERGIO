...
def agregar():
    ...
    if texto != "": # Verifica si el texto no está vacío, es decir, si el usuario ha ingresado algo en el campo de entrada
        lista.insert(tk.END, texto) # Si el texto no está vacío, se inserta el texto en la lista utilizando el método insert() de la lista, con tk.END para agregarlo al final de la lista
        entrada.delete(0, tk.END) # Después de agregar el texto a la lista, se borra el contenido del campo de entrada utilizando el método delete() del campo de entrada, con 0 como índice inicial y tk.END como índice final para eliminar todo el texto
...
def eliminar_seleccionado():
    seleccion = lista.curselection() # Obtiene el índice del elemento seleccionado en la lista utilizando el método curselection() de la lista y lo almacena en la variable seleccion

    if seleccion: # Verificar si hay una selección
        lista.delete(seleccion) # Si hay una selección, se elimina el elemento seleccionado de la lista utilizando el método delete() de la lista, pasando el índice de la selección como argumento
        ...
        messagebox.showwarning("Advertencia", "Seleccione un elemento para eliminar") # Si no hay una selección, se muestra un mensaje de advertencia al usuario utilizando el método showwarning() del módulo messagebox, indicando que debe seleccionar un elemento para eliminar
...
def actualizar_botones():
    if lista.size() == 0: # Si la lista está vacía, desactivar botones
        btn_eliminar.config(state="disabled") # Desactiva el botón de eliminar seleccionado utilizando el método config() del botón, estableciendo el estado en "disabled"
        btn_limpiar.config(state="disabled") # Desactiva el botón de limpiar todo utilizando el método config() del botón, estableciendo el estado en "disabled"
    else:
        btn_eliminar.config(state="normal") # Si la lista no está vacía, habilita el botón de eliminar seleccionado utilizando el método config() del botón, estableciendo el estado en "normal"
        btn_limpiar.config(state="normal") # Habilita el botón de limpiar todo utilizando el método config() del botón, estableciendo el estado en "normal"
...
# Botón limpiar
btn_limpiar = tk.Button(frame_botones, text="Limpiar todo", command=limpiar_todo, state="disabled") # Crea un botón para limpiar toda la lista, con el texto "Limpiar todo", asigna la función limpiar_todo() al evento de clic y lo desactiva inicialmente
